        ...
        <div v-if="seller" class="card" style="padding: 12px; display: grid; gap: 6px">
          <div class="row-between">
            <div class="row" style="gap: 8px">
              ...
              />
              <NuxtLink :href="`/profile/${seller.id}`"
                ><strong>{{ seller.display_name }}</strong></NuxtLink
              >
            </div>
            <div class="muted" style="font-size: 12px">
              ⭐️ {{ Math.round((seller.rating_average || 0) * 10) / 10 }} ({{
                seller.review_count
              }})
            </div>
          </div>
...
import { useRoute, useAsyncData, useHead, navigateTo } from '#app'
...
async function markSold(sold: boolean) {
  if (!listing.value) return
  if (!user.value) return navigateTo('/login')
  await $fetch(`/api/listings/${listing.value.id}/status`, {
    method: 'POST',
